...
def run_conversation(audio_files: list[str]) -> dict:
    """Runs a multi-turn conversation loop using pre-recorded audio turn files."""
    load_dotenv()
    db.init_db()
    ...
    print("\n" + "="*60)
    print("      TALKSE STEP 4 — FILE-BASED CONVERSATION PIPELINE      ")
    print("="*60)
    ...
    for idx, audio_file in enumerate(audio_files):
        ...
        if missing is not None:
            ...
            # All fields collected -> Execute Real Booking Engine Action
    ...
    print("\n" + "="*60)
    print("           TALKSE CONVERSATION SUMMARY           ")
    print("="*60)
...
def run_conversation_live(max_turns: int = 6) -> dict:
    """Runs a live-microphone conversation loop with WebRTC VAD and real-time streaming TTS audio playback."""
    if mic is None:
        print("[Error] mic_input module not available. Install sounddevice, numpy, and webrtcvad-wheels.")
        sys.exit(1)
    ...
    os.makedirs("turns", exist_ok=True)
    idempotency_key = f"conv_live_{int(time.time())}_{uuid.uuid4().hex[:6]}"
    ...
    print("\n" + "="*60)
    print("   TALKSE STEP 4 — LIVE MIC INPUT & STREAMING TTS CONVERSATION   ")
    print("="*60)
    print(f"Conversation Idempotency Key: {idempotency_key}")
    ...
    for _ in range(max_turns):
        ...
        print(f"\n--- Turn {turn_num} / {max_turns} (LIVE MICROPHONE TURN) ---")
        
        # 1. Live Mic capture via WebRTC VAD
        audio_file = mic.record_turn(max_seconds=15, silence_threshold_ms=800)
        
        turn_start = time.perf_counter()
        
        # 2. Transcribe speech-only turn audio
        transcript, stt_time = safe_transcribe_with_retry(audio_file)
        if transcript is None:
            print("[Error] STT failed permanently. [WOULD TRANSFER TO HUMAN]")
            state["status"] = "transferred_to_human"
        ...
        print(f"Caller Said: '{transcript.strip()}'")
        ...
        if missing is not None:
            ...
            print(f"Assistant Final Result Reply: '{reply_text}'")
            ttft, tts_time = ap.stream_and_play_tts(reply_text, f"turns/reply_result.wav")
    ...
    # Final Output & Streaming Latency Comparison
    print("\n" + "="*65)
    print("     TALKSE STEP 4 LIVE MIC & STREAMING CONVERSATION SUMMARY     ")
    print("="*65)
    print(f"Final State Dict:\n{json.dumps(state, indent=2, default=str)}\n")
    ...
    print("-" * 65)
    print("STREAMING TTS PER-TURN LATENCY LOG (PERCEIVED RESPONSIVENESS):")
    print("Turn | STT (s) | LLM (s) | Time To First Audio (TTFT) | Total Turn (s)")
    print("-" * 65)
    for log in latency_log:
        warn_str = "YES (>4s TTFT)" if log["exceeded_threshold"] else "OK"
        print(f"  {log['turn']:2d} |  {log['stt_time']:6.3f} |  {log['llm_time']:6.3f} |          {log['ttft']:6.3f} s         |     {log['total_turn_time']:6.3f}     | {warn_str}")
        
    print("-" * 65)
    print(f"TOTAL LIVE CONVERSATION RUN TIME: {total_conversation_time:6.3f} s")
    print("="*65 + "\n")
...
if __name__ == "__main__":
    if "--live" in sys.argv or "-l" in sys.argv:
        run_conversation_live()
    elif len(sys.argv) > 1:
        files = [f for f in sys.argv[1:] if not f.startswith("-")]
        run_conversation(files)
    else:
        run_conversation(["turns/book_t1.wav", "turns/book_t2.wav", "turns/book_t3.wav"])

...
def get_groq_client():
    ...
    if _groq_client is None:
        ...
        _groq_client = Groq(api_key=groq_key, timeout=float(os.getenv("LLM_TIMEOUT_SECONDS", "4.0")))
    return _groq_client
...
def transcribe(audio_path: str) -> tuple[str, float]:
    """Transcribes an audio file using Groq's whisper-large-v3-turbo model (sub-second latency)."""

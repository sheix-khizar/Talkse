...
def insert_appointment(idempotency_key: str, service_id: str, provider_id: str, caller_name: str, scheduled_start, scheduled_end, deposit_required: bool = False, caller_phone: str = None) -> dict:
    ...
    Raises SlotTakenError if a concurrent request already booked this provider/slot
    (caught by the uq_provider_slot unique index)."""
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            try:
                ...
                conn.rollback()
                print(f"[DB] Slot for provider '{provider_id}' at {scheduled_start} was claimed concurrently.")

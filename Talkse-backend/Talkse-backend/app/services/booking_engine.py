...
def check_emergency_protocol(tenant_id: str, text: str) -> str | None:
    if not text:
        return None
    lower = text.lower()
    emergency_keywords = [
...
def book_appointment(tenant_id: str, state: dict, idempotency_key: str) -> dict:
    ...
    valid_providers = config.providers_for_service(tenant_id, service["id"])
...
def cancel_appointment(tenant_id: str, existing_ref: str) -> dict:
    appt = db.get_appointment(tenant_id, existing_ref)
    if not appt:
        return {"status": "rejected", "reason": f"No active appointment found for reference '{existing_ref}'."}

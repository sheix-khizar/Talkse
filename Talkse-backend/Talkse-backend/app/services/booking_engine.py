...
def parse_time_string(time_str: str) -> datetime | None:
    ...
    If the caller only gave a time-of-day (e.g. '2pm') and dateutil defaults it to
    today but that time has already passed, roll forward to tomorrow at that time.
    If parsing fails entirely, fall back to tomorrow at 2pm."""
    if not time_str:
        return None
    now = datetime.now()
    try:
    ...
    if dt < now:
        # dateutil defaulted the date to "now" because only a time was given,
        # and that time-of-day has already passed today -> push to tomorrow,
        # same time-of-day.
...
def book_appointment(state: dict, idempotency_key: str) -> dict:
    ...
    # 3. Resolve provider — try every eligible provider, not just the first
    ...
    provider = None
    reason = "No providers were available at the requested time."
    for candidate in valid_providers:
        candidate_start = requested_start

        # Ensure requested start falls on a valid working day for this candidate
        if not config.provider_works_on(candidate["id"], candidate_start):
            for offset in range(1, 8):
                test_dt = requested_start + timedelta(days=offset)
                if config.provider_works_on(candidate["id"], test_dt) and config.is_clinic_open(test_dt):
                    candidate_start = test_dt
                    break

        available, candidate_reason = is_slot_available(candidate["id"], service["id"], candidate_start)
        if available:
            provider = candidate
    ...
    # 7. Insert row into Postgres (handle race against a concurrent booking of the same slot)
    try:
